...
def _build_leakage_safe_split(rows: list[ProcessedRow], settings: Settings) -> LeakageSafeSplit:
    ...
    fit_cut = max(1, int(len(outer_split.train) * 0.8))
    if fit_cut >= len(outer_split.train):
        fit_cut = max(1, len(outer_split.train) - 1)
    fit_train = outer_split.train[:fit_cut]
    fit_validation = outer_split.train[fit_cut:]
    return LeakageSafeSplit(
        fit_train=fit_train,
        fit_validation=fit_validation,
        selection=outer_split.validation,
        final_test=outer_split.test,
        reports={
            "fit_train_rows": len(fit_train),
            "fit_validation_rows": len(fit_validation),
            "selection_rows": len(outer_split.validation),
            "final_test_rows": len(outer_split.test),
            "leakage_safe": True,
        },
    )
    ...
    for threshold in settings.threshold.grid:
        ...
        sweep.append(
            ThresholdSweepResult(
                ...
                metric_value=float(metrics.get("expectancy_usd", 0.0)),
                total_trades=int(metrics.get("total_trades", 0)),
                blocked_entry_count=int(metrics.get("blocked_entry_count", 0)),
                net_pnl_usd=float(metrics.get("net_pnl_usd", 0.0)),
                profit_factor=float(metrics.get("profit_factor", 0.0)),
                max_drawdown_usd=float(metrics.get("max_drawdown_usd", 0.0)),
...
def _symbol_walkforward(
    ...
    for window in windows:
        ...
        if metrics.get("net_pnl_usd", 0.0) > 0.0:
            positive += 1
        folds.append(
            {
...
def run_strategy_validation(settings: Settings) -> int:
    ...
    if not all_rows:
        logger.error("No hay filas procesadas en timeframe primario para validacion por simbolo")
        return 1
    ...
    strategy_validation_report: dict[str, Any] = {"symbols": {}}
    model_comparison_report: dict[str, Any] = {"symbols": {}}
    dynamic_exit_report: dict[str, Any] = {"symbols": {}}
    threshold_report: dict[str, Any] = {"symbols": {}}
    symbol_enablement_report: dict[str, Any] = {"symbols": {}}
    strategy_block_root_cause_report: dict[str, Any] = {"symbols": {}}
    symbol_block_diagnostics_report: dict[str, Any] = {"symbols": {}}
    threshold_sensitivity_report: dict[str, Any] = {"symbols": {}}
    gate_failure_matrix: dict[str, Any] = {"symbols": {}}
    session_timeframe_diagnostics_report: dict[str, Any] = {"symbols": {}}
    exit_policy_diagnostics_report: dict[str, Any] = {"symbols": {}}
    symbol_recommendation_report: dict[str, Any] = {"symbols": {}}
    ...
    for symbol, symbol_rows in sorted(grouped.items()):
        ...
        if len(symbol_rows) < 30:
            ...
            symbol_enablement_report["symbols"][symbol] = decision
        ...
        model_comparison_report["symbols"][symbol] = {
            "chosen_model": chosen_model,
            "comparison": comparison,
        }
        dynamic_exit_report["symbols"][symbol] = {
            "preferred_exit_policy": chosen_payload["preferred_exit_policy"],
            "static": chosen_payload["static"],
            "atr_dynamic": chosen_payload["atr_dynamic"],
        }
        threshold_report["symbols"][symbol] = {
            "chosen_model": chosen_model,
            "thresholds": chosen_payload["thresholds"],
        }
        threshold_sensitivity_report["symbols"][symbol] = {
            "chosen_model": chosen_model,
            "chosen_exit_policy": chosen_payload["preferred_exit_policy"],
        ...
        strategy_validation_report["symbols"][symbol] = {
            "chosen_model": chosen_model,
            "chosen_exit_policy": chosen_payload["preferred_exit_policy"],
            "selection_protocol": symbol_split.reports,
            "walkforward": walkforward,
            "comparison": comparison,
            "decision": decision,
        }
        symbol_enablement_report["symbols"][symbol] = decision
        gate_failure_matrix["symbols"][symbol] = block_matrix
        session_timeframe_diagnostics_report["symbols"][symbol] = session_timeframe_diagnostics
        exit_policy_diagnostics_report["symbols"][symbol] = exit_policy_diagnostics
        symbol_recommendation_report["symbols"][symbol] = {
            "symbol": symbol,
            "current_validation_state": decision["state"],
            "dominant_reason": block_matrix["dominant_reason"],
            **recommendation,
        }
        strategy_block_root_cause_report["symbols"][symbol] = {
            "symbol": symbol,
            "current_validation_state": decision["state"],
            "chosen_model": chosen_model,
            "chosen_exit_policy": chosen_payload["preferred_exit_policy"],
        ...
        symbol_block_diagnostics_report["symbols"][symbol] = {
            "symbol": symbol,
            "selection_protocol": symbol_split.reports,
            "decision": decision,
        ...
        chosen_threshold = chosen_payload["thresholds"][chosen_payload["preferred_exit_policy"]]["threshold"]
        symbol_profile_overrides[symbol] = {
            ...
            "profile_id": f"{symbol}-{run_dir.name}",
    ...
    leakage_fix_report = {
        ...
        "test_used_for_selection": False,
        "global_split": global_split.reports,
        "symbols": {
            symbol: {
    ...
    write_json_report(run_dir, "strategy_validation_report.json", wrap_artifact("strategy_validation", strategy_validation_report))
    write_json_report(run_dir, "model_comparison_report.json", wrap_artifact("model_comparison", model_comparison_report))
    write_json_report(run_dir, "dynamic_exit_report.json", wrap_artifact("dynamic_exit_report", dynamic_exit_report))
    write_json_report(run_dir, "threshold_report.json", wrap_artifact("threshold_report", threshold_report))
    write_json_report(run_dir, "symbol_enablement_report.json", wrap_artifact("symbol_enablement", symbol_enablement_report))
    write_json_report(run_dir, "aggregated_portfolio_report.json", wrap_artifact("strategy_validation", aggregate_portfolio))
    write_json_report(run_dir, "strategy_block_root_cause_report.json", wrap_artifact("strategy_validation_audit", strategy_block_root_cause_report))
    write_json_report(run_dir, "symbol_block_diagnostics_report.json", wrap_artifact("strategy_validation_audit", symbol_block_diagnostics_report))
    write_json_report(run_dir, "threshold_sensitivity_report.json", wrap_artifact("strategy_validation_audit", threshold_sensitivity_report))
    write_json_report(run_dir, "gate_failure_matrix.json", wrap_artifact("strategy_validation_audit", gate_failure_matrix))
    write_json_report(run_dir, "session_timeframe_diagnostics_report.json", wrap_artifact("strategy_validation_audit", session_timeframe_diagnostics_report))
    write_json_report(run_dir, "exit_policy_diagnostics_report.json", wrap_artifact("strategy_validation_audit", exit_policy_diagnostics_report))
    write_json_report(run_dir, "symbol_recommendation_report.json", wrap_artifact("strategy_validation_audit", symbol_recommendation_report))
...
def symbol_go_no_go(settings: Settings) -> int:
    ...
    if latest is None:
        logger.info("No hay validacion previa; ejecutando validacion completa")
        return run_strategy_validation(settings)
    report_path = latest / "symbol_enablement_report.json"
    if not report_path.exists():
        logger.error("No existe symbol_enablement_report.json en %s", latest)
        return 1
